...
import { ArrowDown, ArrowLeft, ArrowUp, BookOpen, ChevronLeft, Check, ChevronDown, Cpu, Home, Lightbulb, Plug, ShieldCheck, Wifi, Zap, Edit3, FolderPlus, ImagePlus, LayoutDashboard, Link2, Package, Plus, RefreshCw, Save, Search, Trash2, Wrench, X } from "lucide-react";
...
const serviceIconOptions = [
  { name: "Wrench", label: "Cờ lê", Icon: Wrench },
  { name: "Cpu", label: "Thiết bị", Icon: Cpu },
  { name: "Zap", label: "Điện", Icon: Zap },
  { name: "Lightbulb", label: "Chiếu sáng", Icon: Lightbulb },
  { name: "ShieldCheck", label: "An toàn", Icon: ShieldCheck },
...
export default function DashboardPage() {
  ...
  const sortButton = (type: ItemType) => (
    <button type="button" onClick={() => sortingType === type ? applySorting(type) : setSortingType(type)} className={`inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-xs font-bold transition-colors ${sortingType === type ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-300" : "border-white/10 text-zinc-300 hover:border-[#ff5017]/50 hover:text-white"}`}>
  ...
  const categoryManager = () => <section className="mt-6 rounded-2xl border border-white/5 bg-[#1a1a1e] p-5">
    <div className="mb-5 flex items-center justify-between gap-4 border-b border-white/10 pb-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <button type="button" onClick={() => { setEditingCategoryId(null); setCategoryForm({ name: "" }); setCategoryFormOpen(true); }} className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-[#ff5017] px-4 py-2.5 text-xs font-bold text-white hover:bg-orange-700">
          <Plus className="h-4 w-4" /> Thêm danh mục
        </button>
      </div>
    </div>
    {categoryFormOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4"><div className="w-full max-w-lg rounded-2xl border border-[#ff5017]/20 bg-[#17171b] p-4 shadow-2xl md:p-5"><FormHeading title={editingCategoryId ? "Chỉnh sửa danh mục" : "Thêm danh mục mới"} onClose={() => setCategoryFormOpen(false)} /><input autoFocus className={inputClass} value={categoryForm.name} onChange={(e) => setCategoryForm({ name: e.target.value })} placeholder="Tên danh mục" /><div className="mt-5 flex justify-end gap-3"><button type="button" onClick={() => setCategoryFormOpen(false)} className="rounded-xl border border-white/10 px-4 py-3 text-sm font-bold text-zinc-400 hover:text-white">Huỷ</button><button type="button" onClick={addCategory} className="rounded-xl bg-[#ff5017] px-5 py-3 text-sm font-bold text-white hover:bg-orange-700">{editingCategoryId ? "Cập nhật" : "Lưu danh mục"}</button></div></div></div>}
    {categories.length === 0 ? <Empty text="Chưa có danh mục." /> : <div className="space-y-3">{categories.map((item, index) => <div key={item.id} draggable={sortingType === "categories"} onDragStart={sortingType === "categories" ? () => setDraggedItem({ type: "categories", index }) : undefined} onDragOver={sortingType === "categories" ? (event) => event.preventDefault() : undefined} onDrop={sortingType === "categories" ? () => { if (draggedItem?.type === "categories") reorderItem("categories", draggedItem.index, index); setDraggedItem(null); } : undefined} className={`flex items-center gap-3 rounded-xl border border-white/5 bg-zinc-950/35 p-3 ${sortingType === "categories" ? "cursor-grab active:cursor-grabbing" : ""}`}>
      <span className="w-7 text-center text-xs font-black text-[#ff5017]">{String(index + 1).padStart(2, "0")}</span>
      <div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-white">{item.name}</p></div>
