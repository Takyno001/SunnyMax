...
export default function DashboardPage() {
  ...
  return <div className="dashboard-page min-h-screen bg-[#0f0f12] text-white">
    ...
    <main className={sidebarCollapsed ? "lg:pl-20" : "lg:pl-64"}><div className="mx-auto max-w-7xl px-3 py-3 sm:px-5 sm:py-8 md:px-10"><div className="mb-4 grid grid-cols-2 gap-2 rounded-2xl border border-white/10 bg-[#151518] p-2 lg:hidden">{navItems.map(({ id, label, icon: Icon }) => <button key={id} type="button" onClick={() => setSection(id)} className={`flex min-h-11 w-full items-center justify-center gap-2 rounded-xl px-2 py-2 text-[11px] font-bold transition-colors ${section === id ? "bg-[#ff5017] text-white shadow-lg shadow-orange-950/30" : "bg-white/[0.04] text-zinc-400"}`}><Icon className="h-4 w-4 shrink-0" /><span>{label}</span></button>)}</div>{notice && <div className="mb-6 flex items-center gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-300"><Check className="h-4 w-4" />{notice}</div>}{content}</div></main>
...
function FormHeading({ title, onClose, onClear }: { title: string; onClose: () => void; onClear?: () => void }) {
  ...
  return <>
    ...
    {confirmOpen && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm pointer-events-auto">
      <div role="dialog" aria-modal="true" aria-labelledby="clear-confirm-title" className="pointer-events-auto w-full max-w-md rounded-2xl border border-white/10 bg-[#1b1b1f] p-6 shadow-[0_24px_80px_rgba(0,0,0,.65)]">
        ...
        <div className="mt-6 flex justify-end gap-3">
          <button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); setConfirmOpen(false); }} className="rounded-xl border border-white/10 px-4 py-2.5 text-sm font-bold text-zinc-400 transition-colors hover:bg-white/5 hover:text-white">Huỷ</button>
          <button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); onClear?.(); setConfirmOpen(false); }} className="inline-flex items-center gap-2 rounded-xl bg-red-500 px-4 py-2.5 text-sm font-bold text-white transition-colors hover:bg-red-600"><Trash2 className="h-4 w-4" />Xoá toàn bộ</button>
        </div>
      </div>
    </div>}
  </>;
