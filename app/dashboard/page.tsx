...
export default function DashboardPage() {
  ...
  const reorderItem = (type: ItemType, from: number, to: number) => {
    ...
    setProducts(next); writeStoredContent(CONTENT_STORAGE_KEYS.products, next);
  };
  ...
  const renderServiceForm = () => <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/75 p-4"><form onSubmit={saveService} className="my-4 max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-[#ff5017]/20 bg-[#17171b] p-4 shadow-2xl md:p-5"><FormHeading title={editingId ? "Chỉnh sửa dịch vụ" : "Thêm dịch vụ mới"} onClose={closeForm} /><div className="grid gap-4"><Field label="Tên dịch vụ *"><input className={inputClass} value={serviceForm.title} onChange={(e) => setServiceForm({ ...serviceForm, title: e.target.value })} required /></Field><Field label="Mô tả ngắn *"><textarea className={`${inputClass} min-h-24 resize-y`} value={serviceForm.description} onChange={(e) => setServiceForm({ ...serviceForm, description: e.target.value })} required /></Field><Field label="Thông tin chi tiết"><textarea className={`${inputClass} min-h-24 resize-y`} value={serviceForm.detail} onChange={(e) => setServiceForm({ ...serviceForm, detail: e.target.value })} /></Field></div><div className="mt-5 flex justify-end"><button type="submit" className="inline-flex items-center gap-2 rounded-xl bg-[#ff5017] px-5 py-3 text-sm font-bold text-white hover:bg-orange-700"><Save className="h-4 w-4" />{editingId ? "Cập nhật dịch vụ" : "Lưu dịch vụ"}</button></div></form></div>;
