...
import { ArrowDown, ArrowLeft, ArrowUp, GripVertical, Newspaper, ChevronLeft, Check, ChevronDown, Cpu, Home, Lightbulb, Plug, ShieldCheck, Wifi, Zap, Edit3, FolderPlus, ImagePlus, LayoutDashboard, Link2, ShoppingBag, Plus, RefreshCw, Save, Search, Trash2, Wrench, X } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const postList = (compact = false) => {
    ...
    return <section className={`rounded-2xl border border-white/5 bg-[#1a1a1e] p-5 ${compact ? "" : "mt-6"}`}><div className="mb-5 flex items-center justify-between border-b border-white/10 pb-4"><div><h2 className="text-lg font-black">Quản lý bài viết ({visiblePosts.length}{visiblePosts.length !== posts.length ? `/${posts.length}` : ""})</h2><p className="mt-1 text-xs text-zinc-600">Tìm kiếm, sắp xếp, chỉnh sửa hoặc xoá bài viết.</p></div><div className="flex items-center gap-2">{sortButton("posts")}<button type="button" onClick={openPostForm} className="inline-flex items-center gap-2 rounded-xl bg-[#ff5017] px-4 py-2.5 text-xs font-bold hover:bg-orange-700"><Plus className="h-4 w-4" /> Thêm bài viết</button></div></div>{!compact && <div className="relative mb-5"><Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-600" /><input className={`${inputClass} pl-11`} value={postQuery} onChange={(event) => setPostQuery(event.target.value)} placeholder="Tìm theo tiêu đề, chuyên mục, nội dung hoặc link..." /></div>}{(visiblePosts.length === 0 ? <Empty text={posts.length === 0 ? "Chưa có bài viết." : "Không tìm thấy bài viết phù hợp."} /> : <div className="space-y-3">{visiblePosts.map((item, index) => <ContentRow key={item.id} title={item.title} meta={`${item.category} · ${formatVietnamDateTime(item.date)}`} image={item.image} index={index} total={visiblePosts.length} sorting={sortingType === "posts"} onMove={(target) => moveItem("posts", index, target - index)} onEdit={() => editPost(item)} onRemove={() => removeItem("posts", item.id)} onDragStart={() => setDraggedItem({ type: "posts", index })} onDragEnter={() => setDragOverItem({ type: "posts", index })} onDragEnd={() => { setDraggedItem(null); setDragOverItem(null); }} onDrop={() => { if (draggedItem?.type === "posts") reorderItem("posts", draggedItem.index, index); setDraggedItem(null); }} />)}</div>)}</section>;
  };
