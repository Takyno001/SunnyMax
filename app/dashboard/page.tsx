...
export default function DashboardPage() {
  ...
  return <div className="min-h-screen bg-[#0f0f12] text-white">
    ...
    <main className={sidebarCollapsed ? "lg:pl-20" : "lg:pl-64"}><div className="mx-auto max-w-7xl px-4 py-5 sm:px-5 sm:py-8 md:px-10"><div className="mb-5 grid grid-cols-2 gap-2 lg:hidden">{navItems.map(({ id, label }) => <button key={id} type="button" onClick={() => setSection(id)} className={`w-full rounded-xl px-3 py-2.5 text-left text-[11px] font-bold ${section === id ? "bg-[#ff5017] text-white" : "bg-white/5 text-zinc-400"}`}>{label}</button>)}</div>{notice && <div className="mb-6 flex items-center gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-300"><Check className="h-4 w-4" />{notice}</div>}{content}</div></main>
